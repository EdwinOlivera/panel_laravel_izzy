...
class OrderAPIController extends Controller
{
    ...
    private function paymentFac(Request $request)
    {
        ...
        try {
            $order = $this->orderRepository->create(
                $request->only('user_id', 'order_status_id', 'tax', 'delivery_address_id', 'delivery_fee', 'hint', 'orden_programada', 'fecha_entrega', 'hora_entrega', 'descripcion', 'order_number',
                    'fecha_creacion',
                    'enable_factura',
                    'nombre_user',
                    'id_factura',
                    ...
                    'instructionGeneral',
                    'rtn_user',
                    'mapa_fechas_cambio', 'carts', 'discount', 'payment_gateway')
            );
            Log::info($input['products']);
            foreach ($input['products'] as $productOrder) {
                $productOrder['order_id'] = $order->id;
