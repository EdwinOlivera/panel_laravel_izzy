...
<div style="flex: 80%;max-width: 80%;padding: 0 4px;" class="column">
    ...
    'class' => 'col-3
        ...
        <div class="col-5">
            {!! Form::number('cant_selectable', null, ['class' => 'form-control', 'min=1', 'placeholder' => trans('lang.group_number_of_items_placeholder')]) !!}
            <div class="form-text text-muted">
    ...
    @if (isset($optionGroup->name_market))

        <div class="form-group row ">
            {!! Form::label('market_id', trans('Esteblecimiento asociado'), ['class' => 'col-3 control-label text-right']) !!}
            <div class="col-9">
                <p>{!! $optionGroup->name_market !!}</p>
                <div class="form-text text-muted">
                    {{ trans('Establecimiento donde se usara este grupo de opciones. Solo productos de este establecimiento podran usar el grupo de opciones') }}
                </div>
            </div>
        </div>
    @else
